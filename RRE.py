...
def number_Nu_edge(t_air, dt_max, dell, l):
    ...
    t_air - температура среды
    """
    c = number_Gr(t_air, dt_max, dell) * dell/l    # Грасгофф. определяющий размер - половина расстояния между рёбер. Т = 50
    q = 12.84 + c
    nu = 6 * c/q * 1/(1 + math.sqrt(1 + (51.4 * c)/q ** 2))   # число нуссельта
    return nu
...
def main():
    ...
    for i in range(n):
        ...
        fr1 += 2 * SV[kk] * h1 * k1     # боковая поверхность рёбер, изымаемая при выборке
